  ...
  const fetchTests = async () => {
    ...
    try {
      const [upcomingRes, pastRes] = await Promise.all([
        api.get(`/student/upcoming-tests/${studentId}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        }),
        api.get(`/student/past-tests/${studentId}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        })
      ...
      if (upcomingRes.data.success) {
        setTests(upcomingRes.data.data || []);
      }

      if (pastRes.data.success) {
        setPastTests(pastRes.data.data || []);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        {/* Past Tests */}
        <div className="mt-10">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Past Tests</h2>
          {pastTests.length === 0 ? (
            <div className="bg-white border-2 border-gray-200 rounded-2xl shadow-md p-8 text-center">
              <p className="text-gray-600">No past tests found</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pastTests.map((test) => (
                <div
                  key={test._id}
                  className="bg-gray-50 border-2 border-gray-200 rounded-2xl shadow-sm p-6"
                >
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="text-xl font-bold text-gray-900 flex-1 pr-3">{test.testTitle}</h3>
                    <span className="px-3 py-1 text-xs font-medium rounded-full whitespace-nowrap bg-red-100 text-red-700">
                      Ended
                    </span>
                  </div>

...
export default function FacultyAnalytics() {
  ...
  useEffect(() => {
    console.log('Faculty Analytics Token Check:', {
      tpo_token: localStorage.getItem('tpo_token') ? 'exists' : 'missing',
  ...
  const fetchData = async () => {
    ...
    try {
      console.log('Fetching faculty analytics for subject:', subject);
      console.log('Using token:', token ? 'exists' : 'missing');
      
      const response = await axios.get(
        `${API_BASE_URL}/tpo/faculty-analytics/complete?subject=${encodeURIComponent(subject)}`,
        {
          headers: {
      ...
      console.log('Faculty Analytics Response:', response.data);
      if (response.data.data) {
        setData(response.data.data);
        console.log('Top Performers:', response.data.data.topPerformers);
      }
    } catch (error) {

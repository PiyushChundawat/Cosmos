...
// 1) Upcoming tests for a student (by college) - show upcoming + active (not ended)
exports.getUpcomingTests = async (req, res) => {
  try {
    ...
    // Show tests that haven't ended yet (both upcoming AND active)
...
// 1b) Past tests for a student (by college) - ended tests
exports.getPastTests = async (req, res) => {
  try {
    ...
    console.error("getPastTests error:", error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.submitTestAttempt = async (req, res) => {
  try {
    ...
    questions.forEach((q) => {
      ...
      // Extract subject from first question
      if (testSubject === 'Unknown' && q.tags?.subject) {
        testSubject = q.tags.subject;
      }
    });
